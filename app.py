...
def train_all_models(_pbp_df, _weekly_df):
    ...
    if 'binary' in models_dict:
        ...
        try:
            st.write("🧠 Training Neural Win Probability Model...")
        ...
        try:
            st.write("🧠 Training Neural Player Models...")
...
def display_home_page(pbp_df, weekly_df, models_dict):
    st.header("Welcome to the Enhanced NFL Prediction System 🚀")
    st.markdown("""
    ...
    **Navigate using the sidebar to:**
    ...
    if models_dict:
        st.success("✅ All data loaded and models trained successfully!")
    else:
        st.warning("⚠️ Models are still training or an error occurred. Please check logs.")
...
def display_spread_tab(models_dict):
    st.header("📊 Point Spread Prediction")
...
def display_player_yardage_tab(models_dict, weekly_df):
    st.header("🏃 Player Yardage Prediction")
...
def display_neural_tab(models_dict):
    ...
    with tab1:
        st.subheader("🏈 Neural Game Outcome Prediction")
    ...
    with tab2:
        st.subheader("🏃 Neural Player Yardage Prediction")
